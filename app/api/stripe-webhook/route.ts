...
export async function POST(req: Request) {
  ...
  switch (event.type) {
    case "checkout.session.completed": {
      ...
      const items: Array<{ productId: string; quantity: number; price: number }> = session.metadata?.items
      ...
      const [newOrder] = await db
        ...
        console.log(items[0].price,'price');
...
// // ✅ Mandatory for Stripe in App Router
// export const runtime = "nodejs"; // Prevents "edge" runtime
// export const dynamic = "force-dynamic"; // Ensures correct streaming

// // ✅ RAW BODY READER — WORKS 100%
